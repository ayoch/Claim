...
SHIP_NAMES = [
    "Headstrong in the Gale", "Lambent Saffron", "Alleged Street Poet",
    "Liberty She Pirouette", "Abandon Propriety", "Anomaly Point",
    "Sacred Bullshit", "Grave Goods", "Frogblast the Vent Core",
    "They're Everywhere", "Phaistos", "Jeirrut", "Searing Epiphany",
    "Squandered Fortune", "Song of Many", "Laziness in Action",
    "Wandering Minstrel", "Faith Like a Candle", "Slurry", "Dastardly Cur",
]


@router.post("/rename-ships")
@limiter.limit("1/hour")
async def rename_ships(request: Request, db: AsyncSession = Depends(get_db)):
    """Rename all ships to use proper ship names from SHIP_NAMES"""
    ...
    for ship in ships:
        available = [n for n in SHIP_NAMES if n not in used_names]
        if not available:
            available = SHIP_NAMES.copy()
            used_names.clear()

        new_name = random.choice(available)
        used_names.add(new_name)
        ship.ship_name = new_name
        renamed += 1
    ...
    return {
        "status": "success",
        "ships_renamed": renamed,
        "message": f"Renamed {renamed} ships"
    }

...
# Rescue debug logging
var _rescue_log_file: FileAccess = null
...
func _ready() -> void:
	...
	if _rescue_log_file:
		_rescue_log_file.store_line("=== Rescue Debug Log Started ===")
		_rescue_log_file.flush()
	...
	for mission: Mission in _missions_buf:
		...
		match mission.status:
					...
					if mission.outbound_waypoint_index < mission.outbound_legs.size():
						...
						match mission.mission_type:
							Mission.MissionType.MINING:
								...
								elif mission.ship.crew.is_empty() or mission.ship.is_derelict:
									# Cannot mine without crew or if derelict
									mission.status = Mission.Status.IDLE_AT_DESTINATION
									mission.ship.position_au = mission.asteroid.get_position_au()
									EventBus.ship_idle_at_destination.emit(mission.ship, mission)
								else:
			...
			Mission.Status.MINING:
				# Check if ship can still mine (has crew and not derelict)
				if mission.ship.crew.is_empty() or mission.ship.is_derelict:
					# Lost crew or became derelict during mining - stop and go idle
					mission.status = Mission.Status.IDLE_AT_DESTINATION
					mission.elapsed_ticks = 0.0
					if mission.asteroid:
						mission.ship.position_au = mission.asteroid.get_position_au()
					...
					# Stay until hold is full (based on cargo policy); safety timeout at 2x estimated duration
					var effective_policy := GameState.get_cargo_policy(mission.ship)
					var cargo_threshold: float = CompanyPolicy.CARGO_POLICY_THRESHOLDS[effective_policy]
					var cargo_full := mission.ship.get_cargo_total() >= mission.ship.get_effective_cargo_capacity() * cargo_threshold
							...
							if mission.mission_type == Mission.MissionType.CREW_FERRY:
								...
								for w in mission.ship.crew:
									if w not in mission.ship.crew and w.needs_rotation:
										WorkerManager.add_to_hitchhike_pool(w, station_name, station_pos)
...
func _mine_tick(mission: Mission, dt: float) -> void:
	...
	# Cannot mine without crew or if derelict
	if ship.crew.is_empty() or ship.is_derelict:
		return
			...
			Mission.Status.MINING, Mission.Status.IDLE_AT_DESTINATION, Mission.Status.DEPLOYING, Mission.Status.COLLECTING:
				...
				if target_pos == Vector2.ZERO and mission.asteroid == null:
					_log_rescue("POSITION RESET: %s | Mission has null asteroid! Status: %s | Old pos: (%.3f, %.3f) -> (0, 0)" % [
						ship.ship_name,
						Mission.Status.keys()[mission.status],
						ship.position_au.x,
						ship.position_au.y
					])
	...
	for ship in GameState.ships:
		...
		if collision["hit"]:
			...
			GameState.add_warning(
				"💥 SHIP DESTROYED: %s crashed into %s — all hands lost" % [ship.ship_name, body_name],
				"critical",
				"destruction",
				ship.position_au,
			)
...
func _log_rescue(message: String) -> void:
	# Write to log file immediately (only called for important rescue events, not every frame)
	if _rescue_log_file:
		var timestamp := "T+%.1f" % GameState.total_ticks
		_rescue_log_file.store_line("[%s] %s" % [timestamp, message])
		_rescue_log_file.flush()
...
func _trigger_breakdown(ship: Ship, reason: String) -> void:
	...
	GameState.add_warning(
		"🚨 BREAKDOWN: %s — %s" % [ship.ship_name, reason],
		"critical",
		"breakdown",
		ship.position_au,
	)
...
func _trigger_fuel_depletion(ship: Ship) -> void:
	...
	EventBus.ship_derelict.emit(ship)
	GameState.add_warning(
		"🚨 BREAKDOWN: %s — Fuel depleted" % ship.ship_name,
		"critical",
		"breakdown",
		ship.position_au,
	)
...
func _process_rescues(dt: float) -> void:
	...
	for ship in completed_rescues:
		...
		var ship_exists := ship in GameState.ships
		var pre_rescue_pos := ship.position_au
		var pre_rescue_vel := ship.velocity_au_per_tick

		_log_rescue("RESCUE START: %s | Exists: %s | Pos: (%.3f, %.3f) | Vel: %.6f | Derelict: %s | Mission: %s | Trade: %s" % [
			ship.ship_name,
			ship_exists,
			pre_rescue_pos.x, pre_rescue_pos.y,
			pre_rescue_vel.length(),
			ship.is_derelict,
			"YES" if ship.current_mission else "NO",
			"YES" if ship.current_trade_mission else "NO"
		])
		...
		# Check if position or velocity changed unexpectedly
		var pos_changed := ship.position_au != pre_rescue_pos
		var vel_changed := ship.velocity_au_per_tick != pre_rescue_vel

		_log_rescue("RESCUE END: %s | Pos: (%.3f, %.3f) | Vel: %.6f | Mission: %s | Trade: %s | Changed: pos=%s vel=%s" % [
			ship.ship_name,
			ship.position_au.x, ship.position_au.y,
			ship.velocity_au_per_tick.length(),
			"YES" if ship.current_mission else "NO",
			"YES" if ship.current_trade_mission else "NO",
			pos_changed,
			vel_changed
		...
		if pos_changed or vel_changed:
			_log_rescue("WARNING: State changed during rescue! Delta pos: (%.3f, %.3f) | Delta vel: %.6f" % [
				ship.position_au.x - pre_rescue_pos.x,
				ship.position_au.y - pre_rescue_pos.y,
				ship.velocity_au_per_tick.length() - pre_rescue_vel.length()
			])
...
func _process_life_support(dt: float) -> void:
		...
		for threshold in LIFE_SUPPORT_WARN_THRESHOLDS:
			if pct <= threshold and threshold not in fired:
				...
				if not player_responded:
					...
					# All life support warnings are critical
					GameState.add_warning(
						"🚨 LIFE SUPPORT: %s at %d%% — send rescue!" % [ship.ship_name, pct_int],
						"critical",
						"life_support",
						ship.position_au,
					)
	...
	for ship in _ships_to_destroy_buf:
		# Defensive check: verify ship still exists in fleet
		if ship not in GameState.ships:
			print("WARNING: Ship %s in destruction buffer but not in GameState.ships - skipping" % ship.ship_name)
			continue

		var crew_count := ship.crew.size()
		var rescue_status := "rescue en route" if ship in GameState.rescue_missions else "no rescue"
		print("DESTROYING SHIP: %s with %d crew (life support failure, %s)" % [ship.ship_name, crew_count, rescue_status])
		...
		GameState.add_warning(
			"💀 SHIP DESTROYED: %s — life support failure, all hands lost" % ship.ship_name,
			"critical",
			"destruction",
			ship.position_au,
			GameState.total_ticks,
			ship  # Pass ship ref for validation at delivery
		)

		# Remove the ship and free its name for reuse
		ShipData.release_name(ship.ship_name)
		GameState.ships.erase(ship)
		print("SHIP DESTROYED COMPLETE: %s removed from fleet" % ship.ship_name)
...
func _trigger_starvation(ship: Ship, mission: Mission = null, trade_mission: TradeMission = null) -> void:
	...
	GameState.add_warning(
		"💀 STARVATION: %d crew died on %s — food depleted" % [dead_workers.size(), ship.ship_name],
		"critical",
		"crew",
		ship.position_au,
	)
...
func _rival_try_dispatch(corp: RivalCorp, ship: RivalShip) -> void:
	...
	# Skill affects mining duration — high skill corps mine longer (fill cargo), low skill leave early
	var skill_duration_mult := 0.6 + (corp.skill * 0.8)  # 0.6x to 1.4x base duration
	ship.mining_duration = RIVAL_BASE_MINING_DURATION * skill_duration_mult * randf_range(0.9, 1.1)
...
func _score_asteroid_for_rival(corp: RivalCorp, ship: RivalShip, asteroid: AsteroidData) -> float:
	...
	# Range filter — skill affects operational range (better navigation/planning)
	var max_range := _get_rival_max_range(ship, corp.skill)
	if dist > max_range:
		return -1.0
	...
	# ── Aggression-Based Competition Tolerance ──────────────────────────────
	# High aggression (0.7-1.0): Willing to contest player claims, ignores competition
	# Mid aggression (0.3-0.7): Some tolerance for competition
	# Low aggression (0.0-0.3): Strongly avoids contested bodies
	var contest_penalty := 1.0
	if player_units > 0:
		if corp.aggression < 0.3:
			# Low aggression — avoid conflict unless minimal competition
			if player_units >= 2:
				return -1.0  # Refuse to contest if 2+ player units
			contest_penalty = 1.0 + (1.0 - corp.aggression) * 2.0  # Heavy penalty
		elif corp.aggression < 0.7:
			# Mid aggression — balanced tolerance
			contest_penalty = 1.0 + (player_units * (1.0 - corp.aggression))
		# else: high aggression — no penalty, may even prefer competition

	# ── Skill-Based Efficiency ──────────────────────────────────────────────
	# High skill (0.7-1.0): Values ore-per-second efficiency (ore_value / dist)
	# Low skill (0.0-0.3): Poor optimization, overvalues proximity
	var proximity_weight := 1.0
	if corp.skill >= 0.7:
		# Skilled corps optimize ore-per-time-invested
		proximity_weight = dist
	elif corp.skill >= 0.3:
		# Mid skill — linear distance penalty
		proximity_weight = dist * 1.5
	else:
		# Unskilled — quadratic distance penalty (overly cautious about distance)
		proximity_weight = dist * dist

	# Final score: value × slot availability ÷ (distance × contest penalty)
	var score := (ore_value * slot_factor) / (proximity_weight * contest_penalty)

	# Aggression bonus for rich, contested bodies (aggressive corps prefer conflict zones)
	if corp.aggression >= 0.7 and player_units > 0:
		score *= 1.0 + (corp.aggression * 0.5)  # Up to +50% bonus

	# Colony standing penalty — banned corps avoid colony territories
	var nearest_colony := _find_nearest_colony(asteroid.get_position_au())
	if nearest_colony:
		# Banned = immediate disqualification
		if GameState._is_corp_banned_from_colony(corp, nearest_colony):
			return -1.0

		# Each active violation reduces desirability by 20%
		var active_violations := GameState._get_active_corp_violations(corp, nearest_colony, GameState.total_ticks)
		if active_violations > 0:
			score *= (1.0 - active_violations * 0.2)

	return score

func _get_rival_max_range(ship: RivalShip, skill: float) -> float:
	# Skill affects operational range — skilled corps venture farther with better planning
	var base_range := 4.0 + ship.thrust_g * 4.0
	var skill_mult := 0.7 + (skill * 0.6)  # 0.7x to 1.3x base range
	return base_range * skill_mult

func _calc_rival_ore_rate(asteroid: AsteroidData, skill: float) -> float:
	var total := 0.0
	for ore_type in asteroid.ore_yields:
		...
		if skill >= 0.7:
			...
			# Low skill — uses flat estimate, ignores market
			total += rate * 1000.0
	return total
...
func _sell_rival_cargo(corp: RivalCorp, ship: RivalShip) -> int:
	...
	# Base multiplier: rivals sell at 80% of market price
	var base_multiplier := 0.8

	# Penalty for violations at nearest colony (where cargo is sold)
	var nearest_colony := _find_nearest_colony(ship.get_position_au())
	if nearest_colony:
		var active_violations := GameState._get_active_corp_violations(corp, nearest_colony, GameState.total_ticks)
		if active_violations > 0:
			# Each violation reduces price by 5% (floor at 50%)
			base_multiplier -= active_violations * 0.05
			base_multiplier = maxf(base_multiplier, 0.5)

	var revenue := int(ship.cargo_tons * avg_price * base_multiplier)
...
func _check_violation_warnings() -> void:
	...
	for colony in GameState.colonies:
		...
		if colony.player_banned:
			# Banned status warning (with lightspeed delay from colony)
			GameState.add_warning(
				"❌ BANNED from %s" % colony.colony_name,
				"critical",
				"violation",
				colony.get_position_au()
			)
			...
			# Final warning (3/4) with lightspeed delay
			GameState.add_warning(
			...
			# Warning (2/4) with lightspeed delay
			GameState.add_warning(
...
func _check_combat_encounters(dt: float) -> void:
	...
	# Check player ships vs rival ships for potential combat
	# Combat is bidirectional: either side can initiate based on aggression
	for player_ship in GameState.ships:
		if player_ship.is_derelict or player_ship.is_docked:
			continue

		for corp in GameState.rival_corps:
			for rival_ship in corp.ships:
				# Skip inactive rival ships
				if rival_ship.status != RivalShip.Status.MINING and rival_ship.status != RivalShip.Status.TRANSIT_TO and rival_ship.status != RivalShip.Status.TRANSIT_HOME:
					continue

				var distance := player_ship.position_au.distance_to(rival_ship.get_position_au())

				if distance > COMBAT_ENCOUNTER_RANGE:
					continue  # Too far apart

				# Check aggression from both sides
				var player_aggressive := player_ship.aggression_stance == Ship.AggressionStance.AGGRESSIVE

				# Rival corps weigh risks before attacking
				var rival_aggressive := _rival_should_attack(player_ship, rival_ship, corp, distance)

				if not player_aggressive and not rival_aggressive:
					continue  # Neither side wants to fight

				# Determine who attacks
				var player_attacks := false
				var rival_attacks := false

				if player_aggressive:
					var player_range := player_ship.get_max_weapon_range()
					if distance <= player_range:
						player_attacks = true

				if rival_aggressive:
					# Rival ships have effective range of ~0.05 AU (short-range weapons)
					if distance <= 0.05:
						rival_attacks = true

				if not player_attacks and not rival_attacks:
					continue  # No one can reach

				# Combat location = midpoint between ships
				var combat_pos := (player_ship.position_au + rival_ship.get_position_au()) / 2.0

				# Determine labels and who initiated
				var attacker_label: String
				var defender_label: String
				var player_initiated := player_attacks and (not rival_attacks or randf() < 0.5)

				if player_initiated:
					attacker_label = "[YOUR] " + player_ship.ship_name
					defender_label = "[%s] Ship" % corp.corp_name
				else:
					attacker_label = "[%s] Ship" % corp.corp_name
					defender_label = "[YOUR] " + player_ship.ship_name

				GameState.add_warning(
					"⚔️ COMBAT: %s engaging %s (%.2f AU)" % [attacker_label, defender_label, distance],
					"critical",
					"combat",
					combat_pos,
					GameState.total_ticks
				...
				# Resolve combat based on who's attacking
				_resolve_bidirectional_combat(player_ship, rival_ship, corp, distance, player_attacks, rival_attacks)

func _rival_should_attack(player_ship: Ship, rival_ship: RivalShip, corp: RivalCorp, distance: float) -> bool:
	# Risk assessment: rival corps evaluate multiple factors before attacking
	# Key insight: Higher aggression corps take MORE risks (ignore threats/disadvantages)

	# Base willingness from aggression rating (0.0-1.0)
	var base_aggression := corp.aggression

	# Factor 1: Distance (closer = less risky, within 0.05 AU is optimal)
	var distance_factor := 1.0
	if distance > 0.05:
		distance_factor = 0.05 / distance  # Penalty for being far

	# Factor 2: Player ship threat assessment
	var player_weapons := player_ship.get_weapons_in_range(distance)
	...
	var threat_factor: float = lerp(base_threat_penalty, 1.0, base_aggression * 0.7)  # High aggression = less concerned

	# Factor 3: Range advantage - if player has long-range weapons and we can't reach, risky
	# Again, aggressive corps care less about range disadvantage
	var range_disadvantage: float = maxf(0.0, player_max_range - 0.05)  # Rival effective range ~0.05 AU
	var base_range_penalty: float = 1.0 / (1.0 + range_disadvantage * 2.0)
	var range_factor: float = lerp(base_range_penalty, 1.0, base_aggression * 0.5)  # High aggression = less concerned

	# Factor 4: Skill confidence (higher skill = more willing to take risks)
	var skill_factor := 0.5 + corp.skill * 0.5  # 0.5-1.0 range

	# Factor 5: Personality modifiers
	var personality_modifier := 1.0
	match corp.personality:
		RivalCorp.Personality.AGGRESSIVE:
			personality_modifier = 1.5  # 50% more likely to attack
		RivalCorp.Personality.CONSERVATIVE:
			personality_modifier = 0.5  # 50% less likely
		RivalCorp.Personality.OPPORTUNISTIC:
			# Only attack if we have clear advantage (close range, low threat)
			if distance < 0.03 and player_weapon_count == 0:
				personality_modifier = 2.0
			else:
				personality_modifier = 0.3
		RivalCorp.Personality.EXPANSIONIST:
			personality_modifier = 1.2  # Slightly more aggressive

	# Calculate final attack probability
	var attack_chance: float = base_aggression * distance_factor * threat_factor * range_factor * skill_factor * personality_modifier

	# Colony militia threat — banned corps risk intervention near colonies
	var nearest_colony := _find_nearest_colony(rival_ship.get_position_au())
	if nearest_colony:
		var distance_to_colony := rival_ship.get_position_au().distance_to(nearest_colony.get_position_au())
		if distance_to_colony < 0.5:  # Within colony defense range
			var active_violations := GameState._get_active_corp_violations(corp, nearest_colony, GameState.total_ticks)
			if active_violations >= 2:
				# High violation count = colony may defend player
				# Reduce attack chance by 20% per violation
				attack_chance *= (1.0 - active_violations * 0.2)

	# Clamp to reasonable range
	attack_chance = clampf(attack_chance, 0.0, 0.95)  # Never 100% certain

	# Roll the dice
	return randf() < attack_chance

func _resolve_bidirectional_combat(player_ship: Ship, rival_ship: RivalShip, corp: RivalCorp, distance: float, player_attacks: bool, rival_attacks: bool) -> void:
	# Bidirectional combat: either or both sides can attack
	var player_damage := 0.0
	var rival_damage := 0.0

	# Player ship attacks
	if player_attacks:
		var atk_weapons := player_ship.get_weapons_in_range(distance)
		for weapon in atk_weapons:
			if weapon.fire_rate == "limited":  # Torpedo
				if weapon.current_ammo > 0:
					var quality_stats := MunitionsData.get_quality_stats(weapon.ammo_quality)
					var reliability: float = quality_stats.get("reliability", 0.95)
					if randf() < reliability:
						player_damage += weapon.weapon_power
						weapon.current_ammo -= 1
						EventBus.torpedo_fired.emit(player_ship, weapon.equipment_name, null)
					else:
						weapon.current_ammo -= 1  # Misfire
			else:
				# Lasers/rail guns
				if randf() < weapon.weapon_accuracy:
					player_damage += weapon.weapon_power

	# Rival ship returns fire
	if rival_attacks:
		# Check for colony militia intervention
		if _check_colony_militia_defense(corp, player_ship.position_au):
			# Militia intercepted — no rival damage
			rival_damage = 0.0
		else:
			# Rival weapon damage based on corp's skill and aggression
			# Base damage 10-30, modified by skill (0.0-1.0)
			var base_damage := randf_range(10.0, 30.0)
			rival_damage = base_damage * (0.5 + corp.skill * 0.5)  # 50%-100% effectiveness
	...
	if player_damage > 0:
		...
		GameState.add_warning(
			...
			"warning",
			"combat",
			rival_ship.get_position_au(),
			GameState.total_ticks
		)

		# Criminal violations for attacking (player initiated unprovoked attack)
		if not rival_attacks:  # Only violation if player attacked first
			var nearest_colony := _find_nearest_colony(player_ship.position_au)
			if nearest_colony:
				_queue_violation(nearest_colony, "unprovoked_attack", player_ship.position_au)
	...
	if rival_damage > 0:
		...
		var nearest_colony := _find_nearest_colony(player_ship.position_au)
		if nearest_colony:
			if not player_ship.is_armed():
				# Attacked unarmed ship - severe violation
				_queue_npc_violation(corp, nearest_colony, "attacked_unarmed_ship", player_ship.position_au)
				_queue_npc_violation(corp, nearest_colony, "attacked_unarmed_ship_major", player_ship.position_au)
			elif not player_attacks:
				# Unprovoked attack (player didn't attack first)
				_queue_npc_violation(corp, nearest_colony, "unprovoked_attack", player_ship.position_au)

		if player_ship.is_derelict:
			GameState.add_warning(
				"🚨 SHIP DISABLED: %s disabled by %s!" % [player_ship.ship_name, corp.corp_name],
				"critical",
				"combat",
				player_ship.position_au,
				GameState.total_ticks
			)
	...
	EventBus.combat_resolved.emit({
		...
		"player_damage_taken": rival_damage,
		"rival_destroyed": player_damage > 0,
		"player_disabled": player_ship.is_derelict
	})
...
func _resolve_combat(attacker: Ship, defender: Ship, distance: float) -> void:
	...
	for weapon in atk_weapons:
		if weapon.fire_rate == "limited":  # Torpedo
			if weapon.current_ammo > 0:
				# Get ammo quality and check reliability
				var quality_stats := MunitionsData.get_quality_stats(weapon.ammo_quality)
				var reliability: float = quality_stats.get("reliability", 0.95)

				if randf() < reliability:
					# Torpedo fires successfully
					...
					# Misfire! Torpedo expended but doesn't launch
					weapon.current_ammo -= 1
					print("MISFIRE: %s on %s failed to launch!" % [weapon.equipment_name, attacker.ship_name])
		else:
			# Regular weapons (lasers, rail guns) - no quality system (yet)
			if randf() < weapon.weapon_accuracy:
	...
	# Phase 3: Evasion (pilot skill + torpedo quality accuracy)
	var defender_pilot_skill := 0.0
	for worker in defender.crew:
		defender_pilot_skill = max(defender_pilot_skill, worker.pilot_skill)

	var base_evasion := defender_pilot_skill * 0.4  # Up to 60% at skill 1.5
	var torpedoes_evaded: Array[Equipment] = []
	for torpedo in incoming_torpedoes:
		# Apply quality accuracy modifier (poor quality = easier to evade)
		var quality_stats := MunitionsData.get_quality_stats(torpedo.ammo_quality)
		var accuracy_mod: float = quality_stats.get("accuracy_mod", 0.0)
		var effective_evasion := base_evasion - accuracy_mod  # Better accuracy = harder to evade
	...
	# Phase 4: Apply torpedo damage (with quality power modifier)
	for torpedo in incoming_torpedoes:
		var quality_stats := MunitionsData.get_quality_stats(torpedo.ammo_quality)
	...
	if atk_damage > 0:
		_apply_combat_damage(defender, atk_damage, is_emp_hit, attacker)
		defender_disabled = defender.is_derelict

	if def_damage > 0:
		_apply_combat_damage(attacker, def_damage * 0.5, false, defender)  # Return fire is less effective
		attacker_damaged = true

	# Phase 7: Criminal violations (queued with lightspeed delay to colony)
	# Only issue violations if attacker is a player ship (not null)
	if attacker != null:
		if not defender.is_armed():
			# Attacking unarmed ship - major violation (2 strikes)
			var nearest_colony := _find_nearest_colony(attacker.position_au)
			if nearest_colony:
				_queue_violation(nearest_colony, "attacked_unarmed_ship", attacker.position_au)
				_queue_violation(nearest_colony, "attacked_unarmed_ship_major", attacker.position_au)
		else:
			# Attacking armed ship - minor violation
			var nearest_colony := _find_nearest_colony(attacker.position_au)
			if nearest_colony:
				_queue_violation(nearest_colony, "unprovoked_attack", attacker.position_au)
...
func _apply_combat_damage(ship: Ship, damage: float, is_emp: bool, attacker: Ship) -> void:
	if is_emp:
		...
		GameState.add_warning(
			"⚡ EMP HIT: %s disabled!" % ship.ship_name,
			"critical",
			"combat",
			ship.position_au,
		)
	...
	var damage_per_equipment: float = damage / max(ship.equipment.size(), 1.0)

	for equip in ship.equipment:
		equip.durability -= damage_per_equipment * randf_range(0.8, 1.2)

	ship.engine_condition = maxf(0.0, ship.engine_condition - damage * 2.0)

	# Crew casualties based on damage
	var casualty_chance: float = clamp(damage / 50.0, 0.0, 0.8)
	...
	for worker in dead_crew:
		var was_in_ship := worker in ship.crew
		var was_in_global := worker in GameState.workers

		ship.crew.erase(worker)
		GameState.workers.erase(worker)
		GameState.total_crew_deaths += 1

		_log_rescue("COMBAT DEATH: %s on %s | Was in ship: %s | Was in global: %s | Removed from both" % [
			worker.worker_name, ship.ship_name, was_in_ship, was_in_global
		])

		EventBus.crew_casualty_combat.emit(ship, worker)
		GameState.add_warning(
			"💀 CREW DEATH: %s killed on %s" % [worker.worker_name, ship.ship_name],
			"critical",
			"combat",
			ship.position_au,
		)

		# Add violation to attacker for causing death (queued with lightspeed delay)
		# Only issue violations if attacker is a player ship (not null)
		if attacker != null:
			var nearest_colony := _find_nearest_colony(attacker.position_au)
			if nearest_colony:
				_queue_violation(nearest_colony, "crew_death_combat", ship.position_au)
	...
	if ship.engine_condition <= 0:
		...
		EventBus.ship_disabled_combat.emit(ship, damage)
		GameState.add_warning(
			"🚨 SHIP DESTROYED: %s is derelict!" % ship.ship_name,
			"critical",
			"combat",
			ship.position_au,
		)
...
func _handle_fusion_torpedo_use(attacker: Ship) -> void:
	# Reputation catastrophe (queued - colonies learn after lightspeed delay)
	for colony in GameState.colonies:
		# Add 4 violations to EVERY colony (queued with realistic delays)
		for i in 4:
			_queue_violation(colony, "fusion_weapon_use", attacker.position_au)
		# Ban will be set when violations arrive at colony

	EventBus.fusion_weapon_used.emit(attacker)
	EventBus.game_over.emit("fusion_weapon")

	# Add critical warning (with lightspeed delay from attack location)
	GameState.add_warning(
		"🚨 FUSION WEAPON USED - BANNED FROM ALL COLONIES",
		"critical",
		"combat",
		attacker.position_au
	)
...
## Queue violation with realistic lightspeed delay from event location to colony
## This ensures colonies don't "magically" know about distant events instantly
func _queue_violation(colony: Colony, reason: String, event_position: Vector2) -> void:
	# Calculate lightspeed delay from event location to colony
	var colony_pos := colony.get_position_au()
	var distance := event_position.distance_to(colony_pos)
	var delay := distance * GameState.LIGHT_SECONDS_PER_AU
	var event_time := GameState.total_ticks

	# Queue violation to fire after colony receives news
	GameState.pending_orders.append({
		"fires_at": GameState.total_ticks + delay,
		"ship": null,
		"label": "violation_report",
		"fn": func():
			# Colony NOW learns of the event and issues violation
			colony.add_violation(reason, event_time)
	})

## Queue NPC violation with lightspeed delay from event to colony
func _queue_npc_violation(corp: RivalCorp, colony: Colony, reason: String, event_pos: Vector2) -> void:
	var distance := event_pos.distance_to(colony.get_position_au())
	var delay := distance * GameState.LIGHT_SECONDS_PER_AU
	var event_time := GameState.total_ticks
	...
	GameState.pending_orders.append({
		"fires_at": GameState.total_ticks + delay,
		"ship": null,
		"label": "npc_violation_report",
		"fn": func():
			GameState.add_corp_violation(corp, colony, reason, event_time)
	})

## Check if colony militia intervenes to defend against banned corp
func _check_colony_militia_defense(corp: RivalCorp, combat_pos: Vector2) -> bool:
	var nearest_colony := _find_nearest_colony(combat_pos)
	if not nearest_colony:
		return false

	var distance_to_colony := combat_pos.distance_to(nearest_colony.get_position_au())
	if distance_to_colony > 0.5:  # Only defend within 0.5 AU
		return false

	if not GameState._is_corp_banned_from_colony(corp, nearest_colony):
		return false

	# 50% chance militia intercepts banned corp
	if randf() < 0.5:
		EventBus.colony_militia_intervened.emit(corp.corp_name, nearest_colony.colony_name)
		GameState.add_warning(
			"🛡️ %s militia defended against %s" % [nearest_colony.colony_name, corp.corp_name],
			"info",
			"combat",
			combat_pos
		)
